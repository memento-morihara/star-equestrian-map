...
<div
    class="title"
    class:no-desc={!marker.options.description &&
        marker.options.markerType !== "respawning"}
>
    <strong>{marker.options.name}</strong>
    {#if stat(marker.options.name)}<img
            src={`./icons/${stat(marker.options.name)}.svg`}
            height="18"
            title={stat(marker.options.name)}
            alt={`${stat(marker.options.name)}`}
        />{/if}
</div>
{#if marker.options.markerType !== "respawning"}<p>
