 ...
 */
/*
  Heavily modified to work with Vite
*/
...
export default class OverlappingMarkerSpiderfier {
  ...
  circleStartAngle = this.twoPi / 12;
  ...
  constructor(map, opts) {
    ...
    for (k in opts) {
      if (!this.hasProp.call(opts, k)) continue;
      v = opts[k];
      this[k] = v;
    }
  ...
  addListener = function (event, func) {
    var base;
    ((base = this.listeners)[event] != null
      ? base[event]
      : (base[event] = [])
    ).push(func);
    return this;
  ...
  trigger = function () {
    ...
    (event = arguments[0]),
      (args = 2 <= arguments.length ? [...arguments].slice(1) : []);
  ...
  spiderfy = function (markerData, nonNearbyMarkers) {
    var bodyPt,
      footLl,
      footPt,
      footPts,
  ...
  unspiderfy = function (markerNotToMove) {
