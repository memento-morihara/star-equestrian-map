...
<div
  class="text-base align-baseline px-2.5 h-full flex flex-col justify-between"
>
  <div class="flex flex-col md:gap-12 gap-7">
    <div class="flex flex-col gap-1">
      ...
      <div class="space-y-3 mx-2">
        {#if window.__TAURI__}
          ...
          >
        {/if}
        <label><input class="checkbox" id="load-images" name="load-images" type="checkbox" bind:checked={$settings.loadImages} /><span class="ml-2">Automatically load images
        ...
        <label>
          <input
            bind:checked={$settings.spiderfyMarkers}
            class="checkbox"
            type="checkbox"
          />
        ...
        <label>
          <input
            bind:checked={$settings.keepSpiderfied}
            class="checkbox"
            class:!bg-surface-400={!$settings.spiderfyMarkers}
            class:border-on-surface-500={!$settings.spiderfyMarkers}
        ...
        <label>
          <input
            bind:checked={$settings.hoverMarkers}
            class="checkbox"
            on:change={changeHoverSetting}
            type="checkbox"
          />
        ...
        <div class="flex flex-col space-y-2">
          <label class="mt-3" for="opacity"
          ><span>Collected marker opacity</span></label
          >
          <RangeSlider
      ...
      <button
        class="btn variant-ringed mt-5 self-start"
        on:click={revertSettings}>Reset to default
      </button
      >
    </div>
    ...
    <div class="flex flex-col gap-1">
      ...
      <button
        class="btn variant-filled-error mt-5 w-1/4"
        on:click={resetCollected}>Reset
      </button
      >
    </div>
  </div>
